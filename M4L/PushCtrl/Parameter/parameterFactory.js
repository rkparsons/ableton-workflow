    ...
    for (var parameterindex = 0; parameterindex < parameterNames.length; parameterindex++) {
        if (parameterNames[parameterindex]) {
            ...
            if (targetParameterName === 'Category') {
                ...
                parameters.push(new ValueParameter(parameterConfig.displayName, apiPath, apiProperty, parameterConfig.unitType, parameterConfig.unitStyle, parameterConfig.inputRange, parameterConfig.displayRange, parameterConfig.scaleCoefficients))
            }
        }
    }
