...
function createDrumLayers(samplesFolder, drumPadName, pathToDrumLayers, drumLayerCount) {
  ...
  return {
    drumLayers: drumLayers,
    drumLayerNames: drumLayerNames
  };
}
...
function createDrumPads(samplesFolder, pathToDrumRack) {
  ...
  for (var i = 0; i < 16; i++) {
    ...
    if (drumPadApi.get('chains')[1]) {
      ...
      var layersResult = createDrumLayers(samplesFolder, drumPadName, pathToDrumLayers, drumLayerCount);
      var pagesResult = createMixerPages(pathToDrumLayers, drumLayerCount);
...
function initLiveApi() {
  var samplesFolder = this.patcher.filepath.slice(0, this.patcher.filepath.lastIndexOf('/')) + '/samples';
  var drumTrack = drumTrackFactory(samplesFolder);
}
